...
export default function Edit({ portfolio_type }) {
    const { data, setData, put, processing, errors } = useForm({
        title: portfolio_type.title || '',
        sort_order: portfolio_type.sort_order || '',
        status: portfolio_type.status === 'active' ? 'active' : 'inactive',
    });

    function handleSubmit(e) {
        e.preventDefault();
        put(route('portfolio-types.update', portfolio_type.id));
    }

    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Edit Portfolio Type</h2>}
        >
            <Head title="Edit Portfolio Type" />
